...
def parse_antismash_output(antismash_output_directory):
    ...
    for gbk_file in glob.iglob(f"{antismash_output_directory}/*region*.gbk"):
        ...
        contig_id = '.'.join((gbk_file_name.split("."))[0:-2])
        bgc_id = '.'.join((gbk_file_name.split("."))[0:-1])
...
# Assign representative member in a BGC Cluster

previous_family_number = -1
family_number_to_index_and_length = []
for index, row in df_bgc_merge_bigscape_clustering_sorted.iterrows():
    # Check if 'Family Number' is NA or not. Note that rows with 'Family Number'=NA are at top of df because of above 'sort_values' function. 
    ...
    if (previous_family_number != -1) and (current_family_number != previous_family_number):
        # For previous family number, get the index having maximum length and assign corresponding BGC as representative
        family_number_to_index_and_length.sort(key=lambda x:x[1], reverse=True)
        # First element in list is representative one
        representative_index = family_number_to_index_and_length[0][0]
        df_bgc_merge_bigscape_clustering_sorted.loc[representative_index, 'representative_member'] = True
        # Remaining elements are non-representative
        for item in family_number_to_index_and_length[1:]:
            df_bgc_merge_bigscape_clustering_sorted.loc[item[0], 'representative_member'] = False
    
        family_number_to_index_and_length = []
    
    family_number_to_index_and_length.append((index, row['length']))
    previous_family_number = current_family_number

# For last `Family Number`    
family_number_to_index_and_length.sort(key=lambda x:x[1], reverse=True)
# First element in list is representative one
representative_index = family_number_to_index_and_length[0][0]
df_bgc_merge_bigscape_clustering_sorted.loc[representative_index, 'representative_member'] = True
...
# Copy/'Create Symbolic link' representative BGCs to a folder
...
# Copy/'Create Symbolic link' for all BGCs to a folder
for gbk_file in glob.iglob(f"{bigscape_input_dir}/*region*.gbk"):
    gbk_file_basename = os.path.basename(gbk_file)
